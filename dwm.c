...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
            ...
            if (ev->x > (x = selmon->ww - (int)TEXTW(stext) + lrpad - getsystraywidth())) {
			    click = ClkStatusText;
			    char *text = rawstext;
			    int i = -1;
			    char ch;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (ulineall || m->tagset[m->seltags] & 1 << i) { /* if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
	        drw_setscheme(drw, scheme[TagFocus]);
			drw_rect(drw, x + ulinepad, bh - ulinestroke - ulinevoffset, w - (ulinepad * 2) + 1, ulinestroke, 1, 0);
        }
		x += w;
	}
...
drawtab(Monitor *m) {
	...
	for(c = m->clients; c; c = c->next){
      ...
      if (c->issticky) {
          ...
          drw_rect(drw, x + boxs + boxCount, boxs, boxw, boxw, m->sel->isfixed, 0);
      }
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		/* make sure the background color stays the savertpadbar */
		wa.background_pixel  = scheme[SchemeNorm][ColBg].pixel;
